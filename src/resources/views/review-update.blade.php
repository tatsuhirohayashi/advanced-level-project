    ...
    <div class="review__content">
        ...
        <div class="review__content-review">
            <div class="review__content-review-rate">
                <form class="review-form" action="{{ route('review.update', $review['id']) }}" method="post" enctype="multipart/form-data">
                    @method('PATCH')
                    @csrf
                    <p class="review__content-review-rate-p">体験を評価してください</p>
                    <div class="rating">
                        <input type="radio" id="star5" name="rate" value="5" {{ old('rate', $review->rate) == 5 ? 'checked' : '' }} />
                        <label for="star5" title="5 stars">★</label>
                        <input type="radio" id="star4" name="rate" value="4" {{ old('rate', $review->rate) == 4 ? 'checked' : '' }} />
                        <label for="star4" title="4 stars">★</label>
                        <input type="radio" id="star3" name="rate" value="3" {{ old('rate', $review->rate) == 3 ? 'checked' : '' }} />
                        <label for="star3" title="3 stars">★</label>
                        <input type="radio" id="star2" name="rate" value="2" {{ old('rate', $review->rate) == 2 ? 'checked' : '' }} />
                        <label for="star2" title="2 stars">★</label>
                        <input type="radio" id="star1" name="rate" value="1" {{ old('rate', $review->rate) == 1 ? 'checked' : '' }} />
            ...
            <div class="review__content-review-img">
                ...
                @if ($review->image_url)
                <div class="review__content-review-img-preview">
                    <p class="review__content-review-img-preview-p">現在の画像:</p>
